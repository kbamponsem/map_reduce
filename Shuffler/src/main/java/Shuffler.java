...
public class Shuffler {
        ...
        try {
            hashMap.forEach((x, y) -> {
                try {
                    ...
                    String fileHashName = String.format("%s/%s-%s.txt", getBaseDirectory("shuffles"), x, localAddr);
    ...
    static ArrayList<String> getMachinesAsList(String machines) throws IOException {
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(machines))) {
    ...
    static void copySimilarFiles(String filename, String ip) throws IOException {
        ProcessBuilder similarFiles = new ProcessBuilder("scp", filename,
                "amponsem" + "@" + ip + ":" + getBaseDirectory("shufflesReceived"));
